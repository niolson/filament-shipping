...
test.describe('Shipping flow', () => {
  ...
  test('displays fake rates and completes shipment', async ({ page }) => {
    ...
    // Verify at least one fake carrier rate is present (rates show service name + price description).
    await expect(page.locator('main')).toContainText(/Ground Advantage|FedEx Ground|UPS Ground|Priority Mail/);
