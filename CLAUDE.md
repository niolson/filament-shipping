...
- Never add `env_file: .env` to app/queue services — it injects OS-level env vars that override the mounted `.env` file. The `env_file: /opt/shared/shared-secrets.env` (with `required: false`) is intentional — it's how shared secrets (Redis password, Google SSO, OAuth broker secret) are injected in production without being copied into each tenant's `.env`.
