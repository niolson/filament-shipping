...
|---|---|---|
| [shopify-shipping-carrier](shopify-shipping-carrier/) | 8 of 11 | Adapter shipped 2026-08-31. `01` gates the purchase-path work and is `ready-for-human` — a label must be bought in the Shopify admin before the API will sell one. `09` **done** 2026-09-04 — Shopify is now a priceless offer beside the rates, behind a per-client opt-in and a confirmation, and out of every automated path; `10` shipped with `postage-source-split/11`. `08` **done** 2026-09-05 — the rollup now records how many of its packages reported a cost, so spend totals say what they exclude and cost-per-package divides by priced packages only; it spun out `12`, the same defect on the billing path, where the number is invoiced rather than displayed. |
