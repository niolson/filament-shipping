...
PolyBag models **excepted / Section II** batteries only. Fully regulated dangerous-goods declarations (UPS `HazMat` container with UN data, DG contract, `DGSignatoryInfo`, `SubVersion: 1701` — see `docs/issues/special-services/ups_shipmentrequest_dry_ice_or_lithium_batteries.json` and `ups_shipmentrequest_hazardous_materials.json` for what that would entail) remain out of scope.
